...
class DealStageHistoryMapper extends AbstractZohoMapper
{
    public function map(array $payload, array $context = []): array
    {
        return array_merge($this->base($payload, $context), [
            ...
            'previous_stage' => $this->value($payload['Previous_Stage'] ?? null), 'moved_to_stage' => $this->value($payload['Moved_To__s'] ?? null), 'stage_duration_days' => $this->integer($payload['Stage_Duration_Calendar_Days'] ?? null), 'occurred_at' => $this->timestamp($payload['Modified_Time'] ?? $payload['Created_Time'] ?? null),
