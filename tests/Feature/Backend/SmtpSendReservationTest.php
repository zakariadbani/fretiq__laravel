...
class SmtpSendReservationTest extends TestCase
{
    ...
    public function test_two_campaigns_on_one_identity_share_hourly_and_daily_caps(): void
    {
        ...
        $one = $service->reserve($identity, $a, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 1, $now);
        $two = $service->reserve($identity, $b, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 2, $now);
        $three = $service->reserve($identity, $a, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 3, $now);
    ...
    public function test_different_sender_identities_have_independent_caps(): void
    {
        ...
        $service = app(SmtpSendReservationService::class);
        $ra = $service->reserve($a, $this->campaign($a), SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 10, $now);
        $rb = $service->reserve($b, $this->campaign($b), SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 11, $now);
    ...
    public function test_campaign_daily_email_target_is_enforced_independently_of_company_limit(): void
    {
        ...
        $now = Carbon::parse('2026-08-10 09:00', 'Europe/Paris');
        $service = app(SmtpSendReservationService::class);
        $one = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 20, $now);
        $two = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 21, $now);
    ...
    public function test_duplicate_source_reuses_one_reservation(): void
    {
        ...
        $service = app(SmtpSendReservationService::class);
        $one = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 30);
        $two = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 30);
    ...
    public function test_reserved_slots_are_evenly_spread_and_roll_into_the_next_business_window(): void
    {
        ...
        $one = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 40, $friday);
        $two = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 41, $friday);
    ...
    public function test_concurrent_reservations_cannot_allocate_the_same_identity_slot(): void
    {
        ...
        $now = Carbon::parse('2026-08-10 09:00', 'Europe/Paris');
        $service = app(SmtpSendReservationService::class);
        $one = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 60, $now);
        $two = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 61, $now);
    ...
    public function test_overdue_reservations_are_claimed_in_order_and_repaced_from_actual_claim_time(): void
    {
        ...
        $first = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 70, $scheduledAt)['reservation'];
        $second = $service->reserve($identity, $campaign, SmtpSendReservation::SOURCE_CAMPAIGN_RECIPIENT, 71, $scheduledAt)['reservation'];
