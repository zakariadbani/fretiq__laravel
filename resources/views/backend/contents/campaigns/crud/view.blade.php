...
<div class="d-flex align-items-center gap-2 mb-6 flex-wrap">
    @can('view campaigns')
        <a href="{{ route('admin.campaigns.index') }}" class="btn btn-sm fw-bold btn-light">
    ...
    @can('edit campaigns')
        <a href="{{ route('admin.campaigns.edit', $model->id) }}" class="btn btn-sm fw-bold btn-primary">
        ...
        <div class="card h-100">
            <div class="card-header border-0 pt-5">
                <h3 class="card-title fw-bolder m-0">
                    <i class="bi bi-megaphone text-primary fs-3 me-2"></i>
                    Informations
                </h3>
            </div>
            <div class="card-body border-top">
                ...
                <div class="row mb-5">
                    ...
                    <div class="col-lg-7">
                        @php
                            $statusCfg = config('global.data.campaign_statuses.' . $model->status);
                        @endphp
                        @if($statusCfg)
                            <span class="badge badge-light-{{ $statusCfg['color'] }}">{{ $statusCfg['label'] }}</span>
                        @else
                            <span class="badge badge-light-secondary">Brouillon</span>
                        @endif
                    </div>
                </div>

                <div class="row mb-5">
                    <label class="col-lg-5 fw-bold text-muted">Type</label>
                    <div class="col-lg-7">
                        @php
                            $typeCfg = config('global.data.schedule_types.' . $model->schedule_type);
                        @endphp
                        @if($typeCfg)
                            <span class="badge badge-light-{{ $typeCfg['color'] }}">{{ $typeCfg['label'] }}</span>
                        @else
                ...
                <div class="row mb-5">
                    <label class="col-lg-5 fw-bold text-muted">Segment</label>
                    <div class="col-lg-7">
                        <span class="fw-semibold">{{ $model->segment?->name ?? '—' }}</span>
                    </div>
                </div>

                <div class="row mb-5">
                    <label class="col-lg-5 fw-bold text-muted">Modèle</label>
                    <div class="col-lg-7">
                        <span class="fw-semibold">{{ $model->template?->name ?? '—' }}</span>
                    </div>
                </div>

                <div class="row mb-5">
                    <label class="col-lg-5 fw-bold text-muted">Expéditeur</label>
                    <div class="col-lg-7">
                        <span class="fw-semibold">{{ $model->senderIdentity?->name ?? '—' }}</span>
                    </div>
                </div>

                <div class="row mb-5">
                    <label class="col-lg-5 fw-bold text-muted">Planifié le</label>
                    <div class="col-lg-7">
                        <span class="fw-semibold">{{ $model->scheduled_at?->format('d/m/Y H:i') ?? '—' }}</span>
                    </div>
                </div>
...
{{-- ── Recipients drill-down (latest run) ─────────────────────────── --}}
