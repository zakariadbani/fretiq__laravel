{{--
    Review workspace body, hosted inline on the batch view page as the
    "À vérifier" tab pane (backend.contents.prospect_batches.crud.view,
    #prospect_batch_review). $hostBatchId is always the batch id — review is
    per-lot only, there is no standalone cross-batch page.

    Not wrapped in @once — only one host includes this per page today. Add @once around the
    @push blocks below if a second host ever renders this on the same page.
--}}
@php
    ...
    $monitorOutcome = isset($monitoredItem) && $monitoredItem !== null
        ...
        : null;
    // 'batch' is implied by the route, not a filter — an empty state must not
    // permanently claim filters are active just because the (invisible,
    // forced) batch filter is set. Same reasoning for 'state': it defaults to
    // 'attention' (or an item-derived pill) whenever the request didn't send
    // ?state= at all, so only a non-'all' state counts as a chosen filter —
    // EXCEPT the silent default must still count when it hides real results
    // ($stateCounts['all'] > 0): otherwise a plain load defaulting to
    // 'attention' with 0 review items but N failed items renders the "all
    // clear" empty state while the "À relancer (N)" pill shows N pending.
    $hasActiveFilter = $filters['reason'] !== null || ($filters['state'] !== 'all' && (($stateExplicit ?? false) || $stateCounts['all'] > 0)) || $filters['q'] !== '' || $filters['item'] !== null;
@endphp
...
<main class="prospect-review" data-review-workspace>
                ...
                @if($monitorOutcome['terminal'])
                    <div class="d-flex flex-wrap gap-2 mt-3" data-review-monitor-actions>
                        @if($monitorShowsCriteria)
                            ...
                            @endcan
                        @endif
                        <a class="btn btn-light btn-sm" href="{{ $nextMonitoredItem ? $reviewPresenter->workspaceUrl($hostBatchId, ['batch' => $nextMonitoredItem->prospect_batch_id, 'item' => $nextMonitoredItem->id]) : $reviewPresenter->workspaceUrl($hostBatchId, ['batch' => $filters['batch']]) }}">Continuer la file</a>
                    </div>
                @endif
                <a
    ...
    {{-- The hero above (batch view page) already carries the page title/status,
         so the pane header is compact — progress bar only, no h2/lead paragraph. --}}
                ...
                <div class="prospect-review-shell">
                        <aside class="prospect-review-queue" aria-label="Entreprises à vérifier">
                            <div class="prospect-review-queue-header">
                                ...
                                <form method="GET" class="prospect-review-filter" action="{{ url()->current() }}#prospect_batch_review">
                                ...
                                <div class="prospect-review-state-filters" aria-label="Filtrer la file par état">
                                    @foreach(['all' => 'Toutes', 'attention' => 'À décider', 'blocked' => 'À relancer'] as $stateValue => $stateLabel)
                                        <a
                                            ...
                                            href="{{ $reviewPresenter->workspaceUrl($hostBatchId, ['batch' => $filters['batch'], 'reason' => $filters['reason'], 'state' => $stateValue, 'q' => $filters['q']]) }}"
                                        >{{ $stateLabel }} ({{ $stateCounts[$stateValue] }})</a>
                                    @endforeach
                                </div>

                                <details class="prospect-review-more-filters mt-3" @if($filters['reason']) open @endif>
                                    <summary>Filtres avancés</summary>
                                    <form method="GET" class="prospect-review-advanced-filter mt-3" action="{{ url()->current() }}#prospect_batch_review">
                                        <input type="hidden" name="state" value="{{ $filters['state'] }}">
                                        ...
                                        <div class="d-flex gap-2">
                                            ...
                                            <a class="btn btn-sm btn-light" href="{{ $reviewPresenter->workspaceUrl($hostBatchId, ['state' => 'all']) }}">Effacer</a>
                                        </div>
                                    </form>
                                </details>
                            </div>
                            ...
                            @if($items->isNotEmpty())
                                <form method="GET" class="prospect-review-mobile-selector" action="{{ url()->current() }}#prospect_batch_review">
                                    @if($filters['reason'])<input type="hidden" name="reason" value="{{ $filters['reason'] }}">@endif
                                    <input type="hidden" name="state" value="{{ $filters['state'] }}">
                                    @if($filters['q'] !== '')<input type="hidden" name="q" value="{{ $filters['q'] }}">@endif
                            ...
                            <div class="prospect-review-queue-list">
                                @foreach($items as $queueItem)
                                    @php
                                        ...
                                        $queueUrl = $reviewPresenter->workspaceUrl($hostBatchId, [
                                            'batch' => $filters['batch'],
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // A-6: the review pane is one of several tabs on the batch page — a
            // popstate entry from before the reviewer ever opened this tab (or
            // while they're looking at another tab) must not fetch-swap a detail
            // pane that isn't even visible. crud-tabs.js owns re-activating the
            // right tab for that case; this listener only acts when the review
            // pane is the one currently shown.
            window.addEventListener('popstate', () => {
                const reviewPane = document.getElementById('prospect_batch_review');
                if (reviewPane && reviewPane.hidden) {
                    return;
                }
                loadDetailPane(window.location.href, false);
            });

            // A-7: the review pane is one of several tab panes and may start
            // hidden — defer to the moment crud-tabs.js actually shows it
            // (shown.bs.tab, dispatched even for the tab that's already active
            // on load) so focus() never fires on a hidden element and the
            // monitor poller never starts before the reviewer can see it.
            const activateReviewPane = () => {
            if (monitor) {
            ...
            const reviewTabLink = document.getElementById('tab_prospect_batch_review');
            reviewTabLink?.addEventListener('shown.bs.tab', activateReviewPane, { once: true });
        });
    </script>
@endpush
