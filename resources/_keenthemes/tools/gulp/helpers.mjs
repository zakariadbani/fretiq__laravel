...
const cssRewriter = (bundle) => {
  const imgRegex = new RegExp(/\.(gif|jpg|jpeg|tiff|png|ico|svg|webp)$/i);
...
const getDemo = () => {
  // get demo from parameters (e.g. --demo1)
  const keys = Object.keys(argv).filter((k) => !["_", "$0"].includes(k));
  const match = keys.join(" ").match(/(demo\d+)/gi);
  return Array.isArray(match) ? match[0] : "";
};

const getTheme = () => {
  // get theme from parameters (e.g. --metronic); must be a known theme folder
  const folders = getFolders(build.config.path.src.split("{theme}")[0]);
  const keys = Object.keys(argv).filter((k) => !["_", "$0"].includes(k));
  const theme = keys.find((k) => /^[a-z]+$/i.test(k) && !/^demo\d+$/i.test(k) && folders.indexOf(k) !== -1);
  return theme || "";
};
...
const bundle = (bundle) => {
  ...
  if (bundle.hasOwnProperty("src") && bundle.hasOwnProperty("dist")) {
    ...
    if ("mandatory" in bundle.src && "optional" in bundle.src) {
      ...
      for (let vendor in vendors) {
        ...
        for (let type in vendorObj) {
          ...
          switch (type) {
            case "fonts": {
              let fontPaths = vendorObj[type].filter((p) => {
                const base = p.replace(/\/\*\*$/, "");
                return fs.existsSync(base);
              });
              if (fontPaths.length > 0) {
                stream = gulp.src(fontPaths, { allowEmpty: true, encoding: false });
    ...
    for (let type in bundle.src) {
      ...
      // Skip glob entries whose base directory does not exist (avoid ENOENT on missing theme/demo folders)
      const filterExistingBases = (paths) =>
        paths.filter((p) => {
          const base = dotPath(p).replace(/\*.*$/, "").replace(/\/$/, "");
          return base === "" || fs.existsSync(base);
        });

      switch (type) {
        case "styles":
          if (bundle.dist.hasOwnProperty(type)) {
            bundle.src[type] = filterExistingBases(bundle.src[type]);
        ...
        case "images":
          if (bundle.dist.hasOwnProperty(type)) {
            let srcList = bundle.src[type];
            if (type === "fonts") {
              // skip font paths whose base dir does not exist (e.g. @fortawesome webfonts missing)
              srcList = srcList.filter((p) => {
                const base = p.replace(/\/\*\*$/, "");
                return fs.existsSync(base);
              });
            }
            if (srcList.length > 0) {
              stream = gulp.src(srcList, { allowEmpty: true, encoding: false });
      ...
      if (bundle.dist.hasOwnProperty(type)) {
        switch (type) {
          ...
          case "folder":
            ...
            break;
          default:
            stream = gulp.src(bundle.src[type], { allowEmpty: true, encoding: false });
...
/** Clear collected streams (for Gulp v5 task re-runs e.g. watch). */
const clearCollectedStreams = () => {
  bundleStreams.length = 0;
  outputFuncStreams.length = 0;
};

/** Return all streams collected by bundle() and outputFunc() for merging (Gulp v5). */
const getCollectedStreams = () => [...bundleStreams, ...outputFuncStreams];
